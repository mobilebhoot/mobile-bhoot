...
# Set Java environment to use Android Studio's JDK
...
build_android() {
    local build_type=$1
    print_status "Building Android APK ($build_type) using Expo..."
    
    # Set up Android environment
    export ANDROID_HOME="/Users/suresh.s/Library/Android/sdk"
    export PATH=$ANDROID_HOME/platform-tools:$PATH
    
    # Build using Expo (which handles Metro bundling and all configurations)
    if [ "$build_type" = "debug" ]; then
        print_status "Building debug APK with Expo..."
        npx expo run:android --no-install
        APK_PATH="android/app/build/outputs/apk/debug/app-debug.apk"
    else
        print_status "Building release APK with Expo..."
        npx expo run:android --variant release --no-install
        APK_PATH="android/app/build/outputs/apk/release/app-release.apk"
    fi
    
    if [ -f "$APK_PATH" ]; then
        print_success "Android APK built successfully: $APK_PATH"
        ...
        cp "$APK_PATH" "./PocketShield-$build_type.apk"
